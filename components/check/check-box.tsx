...
export default function CheckBox({ tour }: { tour: ITour }) {
  ...
  const { data: tariffs } = useQuery({
    queryKey: ["tariffs", tour.id],
    queryFn: () => getTourTariffs(tour.id),
    // enabled: !!tour,
  });
  ...
  useEffect(() => {
    ...
    params.set("language", selectedTourLanguage);
  ...
  const selectedLangLabel =
    tour.languages!.find((l) => l.code === selectedTourLanguage)?.name ??
    "Language";
              ...
              <div className="rounded-2xl bg-[#F3F4F6]">
                ...
                >
                  <div className="flex items-center gap-2 text-gray-700">
                    ...
                    <span className="font-medium text-sm text-gray-900">
                      {selectedLangLabel || "Choose language"}
                    </span>
                  </div>
                  <motion.span
